  ...
  const handleSignup = async (e) => {
    ...
    try {
      const res = await fetch('/api/auth/signup', {
        ...
        body: JSON.stringify({ student_number: studentNumber, password, role }),
      });
      ...
      <div className="max-w-md w-full bg-white rounded-md shadow-md border border-gray-200 p-8">
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-[#1565C0] hover:bg-[#0D47A1] text-white font-bold py-2 px-4 rounded-sm transition-colors text-sm disabled:opacity-70 mt-2"
          >
